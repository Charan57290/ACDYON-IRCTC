...
import { Search, CalendarClock, TrainFront } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col bg-brand-50">
      ...
      <main className="flex-grow pt-32 pb-20 relative isolate flex flex-col justify-center">
        ...
        <div className="absolute inset-0 -z-10 overflow-hidden">
          <div className="absolute top-0 w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-accent-100 via-brand-50 to-brand-50 opacity-60" />
          <div className="absolute top-20 right-10 w-72 h-72 bg-brand-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob" />
          <div className="absolute top-40 right-40 w-72 h-72 bg-accent-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000" />
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="text-center mb-12">
            ...
            >
              Track your train in real-time. View current location, expected delays, and complete timetable.
            </motion.p>
          </div>
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
              <div className="flex-grow relative">
                <div className="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
                  <TrainFront className="h-6 w-6 text-slate-400" />
                </div>
                <input
                  type="text"
                  value={train}
                  onChange={(e) => setTrain(e.target.value)}
                  placeholder="Enter Train Name or Number (e.g. 12951)"
              ...
              <button
                type="submit"
                disabled={train.length < 3 || isSearching}
